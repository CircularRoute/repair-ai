...
let client = null;
function getClient() {
  // 25-minute request timeout: Mark's search-heavy document runs routinely
  // exceed the SDK's 10-minute default (live: competitor-tracker timed out on
  // almost every weekly refresh since launch).
  if (!client) client = new Anthropic({ timeout: 25 * 60 * 1000 });
  return client;
}
...
export async function complete({
  ...
  for (let turn = 0; turn < 10; turn++) {
    const t0 = Date.now();
    // Streaming, not plain create: long search-heavy runs (Mark's documents)
    // die at ~10-15 minutes as non-streaming requests whatever the client
    // timeout says; a stream stays alive for the whole run.
    response = await getClient().messages.stream({
      model,
    ...
    if (process.env.COMPLETE_DEBUG) {
      console.error(`[complete] agent=${agent} turn=${turn} stop=${response.stop_reason} in=${response.usage.input_tokens} out=${response.usage.output_tokens} ${Math.round((Date.now() - t0) / 1000)}s`);
    }
    recordSpend(
      db,
      {
    ...
    // Newline-joined: text blocks from different turns are separate
    // paragraphs. Bare concatenation glued narration to a document's "# "
    // heading and the publish guard rejected a finished document (live bug).
    ...
    // NOTE: do NOT prefill-continue after a max_tokens stop. The API rejects
    // resuming a response that ends in a thinking or code-execution block
    // (live 400s: "final block cannot be thinking", "container_id required").
    // The cure for truncation is a big enough maxTokens; a truncated document
    // is then refused by the publish guard and the old version survives.
