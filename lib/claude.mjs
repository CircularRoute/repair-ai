...
export async function complete({
  ...
  for (let turn = 0; turn < 10; turn++) {
    ...
    if (response.stop_reason === 'max_tokens' && capContinues < 2) {
      ...
      // at most twice: with a properly sized maxTokens this is a rare rescue,
      // and each extra turn re-sends the whole (search-laden) conversation.
      capContinues++;
      convo = [...convo, { role: 'assistant', content: response.content }];
      continue;
    }
