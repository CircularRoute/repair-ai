...
// Member turned notifications off on this device. Retire the subscription,
// never delete; re-subscribing flips it back to active via the upsert above.
export function disableSubscription(db, memberId, endpoint) {
  const result = db
    .prepare("UPDATE push_subscriptions SET status = 'disabled' WHERE endpoint = ? AND memberId = ?")
    .run(endpoint, memberId);
