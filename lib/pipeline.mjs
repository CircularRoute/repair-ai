...
export async function processMessage(db, messageId, { onBlock = null } = {}) {
  ...
  try {
    ...
    const langPref = member?.language || null;
    let text = msg.originalText;
    let transcriptConfidence = null;
    let transcriptAlt = null;

    // 1. Transcribe voice notes. Language hint from the sender's profile.
    if (msg.kind === 'voice' && msg.audioPath) {
      const primary = await transcribe(db, {
        audioPath: msg.audioPath,
        languageHint: langPref || undefined,
      });
      let transcript = primary.text;
      transcriptConfidence = 0.9;

      // Reinforced Azerbaijani path: second pass with conversation context,
      // agreement check, then a Haiku post-edit with the member glossary.
      if (langPref === 'az') {
