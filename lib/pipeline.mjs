...
import { detectLanguageLLM, translateToEnglish, postEditTranscript, classifyVoiceLanguage } from './claude.mjs';
...
// The post-editor must return a transcript, nothing else. If it explains
// itself instead (seen in production: "I need to correct the speech-to-text
// errors..." shown as a member's words), or balloons far beyond the original,
// keep the raw transcript. A wrong transcript is correctable; leaked model
// meta-text in a member's mouth is not acceptable.
const META_LEAK = /(speech.to.text|primary transcript|corrected transcript|alternative transcript|transcription error|I need to correct|I cannot|I can't|as an AI|the instruction|looks like|appears to be|транскрипт|исходный текст)/i;

// Scripts no member language uses (Arabic, Hebrew, CJK, Kana, Hangul): a
// transcript containing them is transcription garbage from noise (ruling 26).
export function looksForeignScript(text) {
  return /[؀-ۿ֐-׿一-鿿぀-ヿ가-힯]/.test(text || '');
}
export function guardPostEdit(original, edited) {
  const out = String(edited || '').trim();
  if (!out) return original;
  if (out.length > original.length * 2 + 80) return original;
  if (META_LEAK.test(out) && !META_LEAK.test(original)) return original;
  return out;
}
...
export async function processMessage(db, messageId, opts = {}) {
  const { onBlock = null } = opts;
  ...
  try {
    ...
    if (msg.kind === 'voice' && msg.transcriptConfidence === 1 && msg.transcript) {
      ...
      if (langs.includes('az')) {
        ...
        if (detectLanguageHeuristic(transcript) === 'az' || langs.length === 1) {
          const glossary = db
            ...
            .all(msg.senderId);
          try {
            const edited = await postEditTranscript(
              db,
              { transcript, altTranscript: transcriptAlt, context, glossary },
              onBlock
            );
            transcript = guardPostEdit(transcript, edited);
          } catch (err) {
      ...
      // Garbled-transcript gate (ruling 26): members speak en/ru/az only. A
      // transcript in a foreign script, or that reads as another language or
      // gibberish, is transcription garbage (noise, broken mic), not speech.
      // Keep it for the admin, but never classify, embed, or show it to Otto
      // as a normal message; Otto explains the mic cause instead.
      let garbled = looksForeignScript(transcript || '');
      if (!garbled && transcript && !detectLanguageHeuristic(transcript)) {
        try {
          garbled = (await classifyVoiceLanguage(db, transcript, onBlock)) === 'other';
        } catch {}
      }
      if (garbled) {
        update({ pipelineStatus: 'garbled', transcriptConfidence: 0.2 });
        logEvent(db, 'pipeline.garbled', { messageId, sample: (transcript || '').slice(0, 60) });
        if (opts.onProcessed) {
          opts.onProcessed(db.prepare('SELECT * FROM messages WHERE id = ?').get(messageId));
        }
        return;
      }
    }
    ...
    // Post-processing hook (Otto's engagement check runs here so voice notes
    // are already transcribed when Otto reads them).
    if (opts.onProcessed) {
      const processed = db.prepare('SELECT * FROM messages WHERE id = ?').get(messageId);
      opts.onProcessed(processed);
    }
  } catch (err) {
