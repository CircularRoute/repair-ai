...
// Shared between the group prompt and the admin chat prompt so Otto never
// misstates his own abilities anywhere (live incident: he denied voice
// support inside a voice note, then argued the same to Rashad in the
// dashboard because only the group prompt had been taught otherwise).
const OTTO_CAPABILITIES =
  'YOUR CAPABILITIES (so you never misstate them): members\' voice notes are transcribed for you, so you ' +
  'hear and understand every voice message; the text you see marked as a voice note IS what the member said. ' +
  'Your replies CAN be and ARE delivered as voice notes (text-to-speech runs outside you): you reply in ' +
  'kind, voice notes get voice replies, text gets text, and an explicit member request ("reply to me in ' +
  'voice") holds for an hour. NEVER claim you cannot listen to voice messages or cannot send them; you have ' +
  'both abilities, the delivery layer handles the audio. If anything earlier in a conversation, INCLUDING ' +
  'your own previous replies, contradicts this, those earlier claims were a since-fixed bug in your ' +
  'self-knowledge: correct yourself plainly instead of repeating them.\n';
...
const OTTO_SYSTEM =
  ...
  OTTO_CAPABILITIES +
  'When a member EXPLICITLY asks you to reply in voice (or asks to switch back to text), in ANY language ' +
  'and in either a voice note or a typed/dictated text, honor it: begin your reply with exactly ' +
  '[MODE:voice] or [MODE:text] on its own, then the reply itself, and confirm naturally in one short ' +
  'clause (no mechanics, no mention of modes or systems). Their stated wish beats the reply-in-kind ' +
  'default: delivery follows it for the next hour or until they ask otherwise.\n' +
  'NEVER GUESS AT A BROKEN MESSAGE: if a voice transcript looks like noise, a stray fragment, or a language ' +
  'that member would not use, the microphone was probably taken by another program on their phone (a call, ' +
  'Siri, CarPlay, car Bluetooth). Say you could not hear the note properly, name that likely cause, and ask ' +
  'them to resend. Never quote the fragment as their words, never invent or infer what they might have said.\n' +
...
export async function generateReply(db, msg, { onBlock = null } = {}) {
  ...
  // Explicit delivery-mode request (ruling 24): record it, strip the marker.
  const marked = parseModeMarker(reply);
  if (marked.mode) {
    setTempVoiceMode(db, msg.senderId, marked.mode);
    reply = marked.text;
...
// --- Explicit delivery-mode requests (ruling 24): "reply to me in voice" ---
// Otto marks them with [MODE:voice|text]; the wish holds for an hour per
// member, or until the member asks otherwise. Stored in ottoState.

export const VOICE_MODE_HOURS = 1;

export function parseModeMarker(text) {
  const m = /^\[MODE:(voice|text)\]\s*/i.exec(text || '');
  if (!m) return { mode: null, text };
  return { mode: m[1].toLowerCase(), text: text.slice(m[0].length).trim() };
}

export function setTempVoiceMode(db, memberId, mode, now = new Date()) {
  const state = getState(db);
  state.voiceModes = state.voiceModes || {};
  state.voiceModes[memberId] = {
    mode,
    until: new Date(now.getTime() + VOICE_MODE_HOURS * 3600 * 1000).toISOString(),
  };
  saveState(db, state);
  logEvent(db, 'otto.voice_mode', { memberId, mode });
}

export function getTempVoiceMode(db, memberId, now = new Date()) {
  const entry = getState(db).voiceModes?.[memberId];
  if (!entry || now.getTime() > new Date(entry.until).getTime()) return null;
  return entry.mode;
}
