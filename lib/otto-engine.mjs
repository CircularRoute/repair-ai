...
export const DEFAULTS = {
  ...
  proactiveSpacingMin: 30,
  lullMin: 10,            // never intervene while humans are actively talking
  exchangeResetMin: 30,   // a gap this long starts a fresh exchange
};
...
// Engagement: the member addressed Otto by name, replied to an Otto message,
// or is continuing a live exchange with Otto.
export function isEngagement(db, msg, now = new Date()) {
  ...
  if (
    state.exchange &&
    state.exchange.memberId === msg.senderId &&
    state.exchange.lastOttoTs &&
    now.getTime() - new Date(state.exchange.lastOttoTs).getTime() < 15 * 60 * 1000
  ) {
    return true;
  }
  return false;
...
export function exchangeGate(db, memberId, now = new Date()) {
  const { cap } = ottoSettings(db);
  const state = getState(db);
  const ex = state.exchange;
  const resetMs = DEFAULTS.exchangeResetMin * 60 * 1000;
...
export function canIntervene(db, now = new Date()) {
  const { muted, proactivePerDay } = ottoSettings(db);
  ...
  if (last && now.getTime() - new Date(last).getTime() < DEFAULTS.proactiveSpacingMin * 60 * 1000) {
