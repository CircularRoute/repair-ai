...
export function ottoSettings(db) {
  return {
    ...
    spacingMin: Number(getSetting(db, 'ottoSpacingMin', DEFAULTS.proactiveSpacingMin)),
    lullMin: Number(getSetting(db, 'ottoLullMin', DEFAULTS.lullMin)),
...
export function canIntervene(db, now = new Date()) {
  const { muted, proactivePerDay, spacingMin, lullMin } = ottoSettings(db);
  ...
  if (last && now.getTime() - new Date(last).getTime() < spacingMin * 60 * 1000) {
  ...
  if (age < lullMin * 60 * 1000) return { ok: false, reason: 'active-conversation' };
