...
export async function maybeRunBobSchedules(db, { onBlock = null, catchUpExtraction = null } = {}) {
  const now = new Date();
  const digestHour = Number(getSetting(db, 'digestHourUtc', 3));
  const synthesisHour = Number(getSetting(db, 'synthesisHourUtc', 4));
  const lastDigest = getSetting(db, 'bobDigestLastRunAt', '');
  if (now.getUTCHours() === digestHour && (!lastDigest || now.getTime() - new Date(lastDigest).getTime() > 20 * 3600 * 1000)) {
    try { await nightlyDigest(db, { onBlock }); } catch (err) { logEvent(db, 'bob.digest_error', { error: err.message }); }
  }

  if (now.getUTCHours() === synthesisHour) {
