...
// Standing instructions from the admin (dashboard only, ruling 7): the
// founder's way of teaching Bob; they shape every Bob run until removed.
export function getBobDirectives(db) {
  return JSON.parse(getSetting(db, 'bobDirectives', '[]'));
}

export function addBobDirective(db, text) {
  const list = getBobDirectives(db);
  list.push(text.slice(0, 400));
  setSetting(db, 'bobDirectives', JSON.stringify(list.slice(-15)));
  logEvent(db, 'bob.directive_added', { text: text.slice(0, 120) });
}

export function removeBobDirective(db, index) {
  const list = getBobDirectives(db);
  if (index >= 0 && index < list.length) {
    list.splice(index, 1);
    setSetting(db, 'bobDirectives', JSON.stringify(list));
    logEvent(db, 'bob.directive_removed', { index });
  }
}

function bobSystem(db) {
  const directives = getBobDirectives(db);
  return BOB_SYSTEM + (directives.length
...
export function shouldRunDailySynthesis(db, now = new Date()) {
  ...
  const minMessages = Number(getSetting(db, 'synthesisMinMessages', 3));
  const minInsights = Number(getSetting(db, 'synthesisMinInsights', 1));
  ...
  const memberMessages = db
    ...
    .get(dayAgo).c;
  if (memberMessages < minMessages) return { run: false, reason: 'quiet-day', memberMessages };
  const newInsights = db
    .prepare("SELECT COUNT(*) AS c FROM insights WHERE status = 'active' AND extractedAt > ?")
    .get(last || '').c;
  if (newInsights < minInsights) return { run: false, reason: 'no-substance', memberMessages, newInsights };
...
export async function bobChat(db, adminMessage, { onBlock = null } = {}) {
  ...
  const reply = await complete({
    ...
    maxTokens: 2000,
    onBlock,
    system: bobSystem(db) + (defs.length
    ...
    messages: [{
      ...
      content:
        ...
        `Rashad says:\n${adminMessage}\n\n` +
        'This is Rashad through his dashboard, so his instructions ARE authoritative. Decide:\n' +
        '- If he is teaching you or changing HOW you should work from now on (priorities, framing, what to ' +
        'watch for, style), output exactly [DIRECTIVE] followed by a one-line English standing instruction.\n' +
        '- If he asks you to regenerate or update one of your documents, output exactly [RERUN:<type>] where ' +
        '<type> is one of problem-map, opportunity-register, product-concepts, roadmap, build-specs, all.\n' +
        '- Otherwise answer his question with citations, in the language he used. If the corpus cannot ' +
        'support an answer, say what is missing.',
    }],
  });

  const directive = /^\[DIRECTIVE\]\s*(.+)/s.exec(reply.trim());
  const rerun = /^\[RERUN:([a-z-]+)\]/.exec(reply.trim());
  let finalReply = reply.trim();
  if (directive) {
    const text = directive[1].trim().slice(0, 400);
    addBobDirective(db, text);
    finalReply = `Understood and applied from now on: "${text}". It shapes every synthesis and answer until you remove it in my section.`;
  } else if (rerun && (DOCUMENT_TYPES.includes(rerun[1]) || rerun[1] === 'all')) {
    finalReply = rerun[1] === 'all'
      ? 'Running the full synthesis now (all five documents plus the memo); give it a few minutes and check Living documents.'
      : `Regenerating the ${rerun[1]} now; the new version will appear under Living documents shortly.`;
    const job = rerun[1] === 'all' ? weeklySynthesis(db, { onBlock }) : generateDocument(db, rerun[1], { onBlock });
    job.catch((err) => logEvent(db, 'bob.chat_rerun_error', { type: rerun[1], error: err.message }));
  }
  db.prepare("INSERT INTO agent_chat (agent, role, content, at) VALUES ('bob', 'agent', ?, ?)")
    .run(finalReply, new Date().toISOString());
  return finalReply;
}
...
export async function maybeRunBobSchedules(db, { onBlock = null, catchUpExtraction = null } = {}) {
  const now = new Date();
  // Schedule hours are in Central Time (Austin), DST-aware.
  const hourCT = Number(now.toLocaleString('en-US', { timeZone: 'America/Chicago', hour: 'numeric', hour12: false })) % 24;
  const digestHour = Number(getSetting(db, 'digestHourCT', 22));
  const synthesisHour = Number(getSetting(db, 'synthesisHourCT', 23));
  const lastDigest = getSetting(db, 'bobDigestLastRunAt', '');
  if (hourCT === digestHour && (!lastDigest || now.getTime() - new Date(lastDigest).getTime() > 20 * 3600 * 1000)) {
    try { await nightlyDigest(db, { onBlock }); } catch (err) { logEvent(db, 'bob.digest_error', { error: err.message }); }
  }

  if (hourCT === synthesisHour) {
