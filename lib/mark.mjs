...
export async function answerFromResearch(db, question, { onBlock = null } = {}) {
  ...
  try {
    ...
    const chunks = rankChunks(db, vector, 40)
      .filter((c) => c.score > 0.25 && String(c.messageId).startsWith('doc:') &&
        markDocs.some((t) => c.messageId === `doc:${t}`))
      .slice(0, 8);
    if (!chunks.length) return null;
    evidence = chunks.map((c) => `[${c.messageId.slice(4)}] ${c.text.slice(0, 350)}`).join('\n');
  } catch {
    return null;
  }
...
export async function markChat(db, adminMessage, { onBlock = null } = {}) {
  ...
  try {
    ...
    excerpts = rankChunks(db, vector, 40)
      .filter((c) => c.score > 0.2 && String(c.messageId).startsWith('doc:'))
      .slice(0, 8)
      .map((c) => `[${c.messageId.slice(4)}] ${c.text.slice(0, 300)}`)
      .join('\n');
  } catch {}
  const history = db
