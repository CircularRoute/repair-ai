...
const MARK_SYSTEM_BASE =
  ...
  'NEVER narrate your process: no "let me search", no "I will now compile", no notes about search limits or ' +
  'what you are about to do. Think silently between searches; the only prose you ever output is the ' +
  'deliverable itself, and a document always begins with a "# " markdown heading. ' +
  BOUNDARY_NOTE;
...
// A publishable market document: real length and markdown structure. Any
// leftover narration before the first heading is cut; output that never
// reaches a heading or stays tiny is refused so the previous version
// survives (live bug: narration fragments were published as v5 documents).
export function cleanMarkDocument(raw) {
  let content = String(raw || '');
  // Accept a heading even when narration got glued right before it without a
  // newline (live: "...notable entries.# Competitor Tracker" was rejected).
  const m = /(^|\n|\.)(#\s)/.exec(content);
  const heading = m ? m.index + m[1].length : -1;
  if (heading > 0) content = content.slice(heading);
  content = content.trim();
  if (heading === -1 || content.length < 600) {
    const err = new Error('run produced no finished document; previous version kept');
    err.code = 'UNFINISHED_DOC';
    throw err;
  }
  return content;
}
...
export async function runMarkDocument(db, type, { onBlock = null } = {}) {
  ...
  const raw = await complete({
    ...
    model: MODELS.sonnet,
    db,
    // Big enough for searches + the full document in ONE turn: every extra
    // turn re-sends the whole search-laden conversation (live incident:
    // 4000 caused truncation, continuations, and a 700K-token run).
    maxTokens: 10000,
  ...
  let content;
  try {
    content = cleanMarkDocument(raw);
  } catch (err) {
    logEvent(db, 'mark.publish_rejected', { type, reason: err.message, sample: String(raw).slice(0, 120) });
    throw err;
  }
...
// Full refresh of all four market documents (used by the schedule and the
// admin's force-refresh button).
export async function runMarkAll(db, { onBlock = null } = {}) {
  const results = {};
  for (const type of MARK_DOC_TYPES) {
    try {
      results[type] = (await runMarkDocument(db, type, { onBlock })).version;
    } catch (err) {
      results[type] = `error: ${err.message}`;
      logEvent(db, 'mark.refresh_error', { type, error: err.message });
      if (err.code === 'SPEND_BLOCKED') break;
    }
  }
...
export async function maybeRunMarkWeekly(db, { onBlock = null } = {}) {
  ...
  const refreshDays = Number(getSetting(db, 'markRefreshDays', 7));
  if (last && Date.now() - new Date(last).getTime() < refreshDays * 24 * 3600 * 1000) {
