...
const WELCOME_TEMPLATES = {
  en: (name, url) => ({
    ...
    text:
      ...
      `To join, type this address into Safari (iPhone) or Chrome (Android) on your phone:\n${url}\n\n` +
      `Important: on iPhone it must be Safari, even if you normally use Chrome or Google, ` +
      `and do not open the address inside the email app itself.\n\n` +
  ...
  ru: (name, url) => ({
    ...
    text:
      ...
      `Chtoby voyti, nabertie etot adres v Safari (iPhone) ili Chrome (Android):\n${url}\n\n` +
      `Vazhno: na iPhone eto dolzhen byt imenno Safari, dazhe esli vy obychno polzuyetes Chrome ili Google. ` +
      `I ne otkryvayte adres vnutri pochtovogo prilozheniya.\n\n` +
  ...
  az: (name, url) => ({
    ...
    text:
      ...
      `Daxil olmaq uchun bu unvani telefonunuzda Safari (iPhone) ve ya Chrome (Android) brauzerinde yazin:\n${url}\n\n` +
      `Vacib: iPhone-da mutleq Safari olmalidir, adeten Chrome ve ya Google ishletseniz bele. ` +
      `Unvani email tetbiqinin ichinde de achmayin.\n\n` +
